...
const HistoryScreen = () => {
  const isFocused = useIsFocused()
    ...
    useEffect(() => {
      (async () => {
        ...
        let historyPendingData:any = await getJSONData(key_setHistoryPendingList);
  ...
  const _refresh = () =>{    
    ...
    // navigation.setParams({ refreshPending: false });
  }
        ...
        try {
            ...
            const data = await callGraphQL(GET_BADGE_SCANNER_HISTORY, {
              ...
              if (emailStatus === 'pending') {
                ...
                if(historyData && historyData.length > 0 || localData && localData.length >0){
  ...
  const renderItem = ({ item }:any) => {  
    return(
      <View style={styles.containerHistoryStyle}>
        ...
        activeOpacity={0.8}
        onPress={() => 
          navigateScreen(navigation, S_HistoryDetailScreen,{itemDetail: item, refresh:_refresh})}
        style={{flex:1,
        padding: 10,
        }}>
          {(item?.firstNam || item?.lastName) &&
        <TextComponent value={item?.firstName + ' ' + item?.lastName} 
            fontSize={16} 
            fontFamily={DMSansBold} 
            color={theme?.theme.BLACK_COLOR} />
          }
<TextComponent value={item?.clientEmail} 
            fontSize={14} 
            fontFamily={DMSansMedium} 
            color={theme?.theme.BLACK_COLOR} />

      <TextComponent value={moment(item?.timestamp).format('DD MMM YYYY, hh:mm A')} 
                  fontSize={14} 
                  fontFamily={DMSansMedium} 
                  color={theme?.theme.DARK_GREY_COLOR} />
                 {item?.notes &&<Spacer height={8}/>}
                  {item?.notes &&<View  style={[styles.notesView,{
  backgroundColor: theme?.theme?.LIGHT_GREY_COLOR
}]}>
<TextComponent value={item?.notes && item?.notes} 
                  fontSize={14} 
                  fontFamily={DMSansMedium} 
                  color={theme?.theme.BLACK_COLOR} />
</View>}
  ...
  const buttonClick = () => {
    ...
    return (
        ...
        //      onPress={async(title:any) => {
        //            if(title === 'Pending'){
        // setIsLoading(true);
        //             await fetchHistoryData('pending');
        //             // await handleGrabValuesForWeekly()
        //            }
        //      }}
            />
            <Spacer height={20} />
    ...
    alignItems: 'center',
      ...
      >
            ...
            {/* <View style={styles.rowStyle}>
                ...
                <Spacer width={40} />
                <Button title="Update First Record" onPress={handleUpdate} />
            </View>
            <Spacer height={20} />
