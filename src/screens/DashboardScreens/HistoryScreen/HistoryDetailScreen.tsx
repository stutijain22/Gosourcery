...
import {getEssentials, goBack} from '../../../utils/utility';
import {
  DMSansBold,
  DMSansMedium,
  DMSansSemiBold,
  ...
  const sendClick = async () => {
    ...
    try {
      ...
      const sentKey =
        ...
        `${itemDetail.clientEmail}_${itemDetail.collectionId}`;
        ...
        <View style={styles.modalViewStyle}>
          <View
            ...
            <TextComponent
              ...
              value={
                itemDetail?.firstName?.charAt(0).toUpperCase() +
                itemDetail?.lastName?.charAt(0).toUpperCase()
              }
            />
            <Spacer width={30} />
          </View>
          ...
          <InfoRow
            label="Business Type"
            value={itemDetail?.businessType || '-'}
          />
          <InfoRow label="Event ID" value={itemDetail?.eventId || '-'} />
          <InfoRow
            label="Collection"
            value={itemDetail?.collectionName || '-'}
          />
          ...
          <InfoRow
            label="Timestamp"
            value={
              itemDetail?.timestamp
                ? moment(itemDetail.timestamp).format('DD MMM YYYY, hh:mm A')
                : '-'
            }
          />
