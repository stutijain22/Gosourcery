...
import {K_DEFAULT, K_NUMERIC, KT_NEXT} from '../../constant/keyboardReturnType';
...
const AddNotesScreen = (props: any): JSX.Element => {
    ...
    (async () => {
      if (userData) {
        ...
        userData?.userFirstName &&
          (await setFirstName(userData?.userFirstName));
        userData?.userLastName && (await setLastName(userData?.userLastName));
        userData?.userBusinessType &&
          (await setBusinessType(userData?.userBusinessType));
        userData?.userCompanyName &&
          (await setCompanyName(userData?.userCompanyName));
        ...
        userData?.userPhoneNumber &&
          (await setPhoneNumber(userData?.userPhoneNumber));
      }
    })();
  }, [userData]);
  ...
  const saveClick = async () => {
    ...
    if (userData && userData?.length > 0) {
      ...
      setModalVisible({
        title: 'Success',
        key: 'Collection info has been shared with the client.',
        value: true,
      });
    } else {
      setIsLoading(false);
  ...
  const validateInputs = () => {
    ...
    if (lastName.length === 0) {
      ...
      firstName.length === 0
        ? ref_to_first_name?.current?.focus()
        : ref_to_last_name?.current?.focus();
      isValid = false;
    } else {
      setLastNameError(false);
    }
